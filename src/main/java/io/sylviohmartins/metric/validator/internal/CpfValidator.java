...
import io.sylviohmartins.metric.validator.Cpf;
...
public class CpfValidator implements ConstraintValidator<Cpf, String> {
    ...
    public boolean isValid(final String cpf, final ConstraintValidatorContext context) {
        ...
        int digit1 = calculateDigit(onlyDigits.substring(0, 9), MULTIPLIERS);
        int digit2 = calculateDigit(onlyDigits.substring(0, 9) + digit1, MULTIPLIERS);

        return (onlyDigits.substring(0, 9) + digit1 + digit2).equals(onlyDigits);
    }

    private int calculateDigit(final String str, final int[] peso) {
        int sum = 0;
        int digit;
